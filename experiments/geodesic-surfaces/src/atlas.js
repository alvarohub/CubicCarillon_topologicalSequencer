...
const matMul2 = (A, B) => mat2(A.a * B.a + A.b * B.c, A.a * B.b + A.b * B.d, A.c * B.a + A.d * B.c, A.c * B.b + A.d * B.d);
...
export class AtlasSurface {
  constructor({ name, kind, faces, embed, sample, description }) {
    this.name = name;
    this.kind = kind;
    this.faces = faces;
    this.embed = embed;
...
export function createFlatTorus() {
  ...
  return new AtlasSurface({
    ...
    kind: 'torus',
    faces: [face],
    description: 'A rectangle with opposite edges glued. Geodesics are straight lines in the atlas and wrap forever.',
    embed: (head) => torusEmbed(head.x, head.y, width, height),
    sample: (head) => sampleField(head.x, head.y, width, height, head.faceId),
  });
}
...
export function createMobiusStrip(turns = 0.5) {
  const width = 4.6;
  const height = 1.25;
  const face = rectangleFace(0, width, height, turns === 0.5 ? 'Mobius chart' : `${Math.round(turns * 360)} degree twist chart`);
  if (turns === 0.5) {
    glue(face, 1, face, 3, mat2(1, 0, 0, -1), [-width, 0]);
    glue(face, 3, face, 1, mat2(1, 0, 0, -1), [width, 0]);
  } else if (turns === 0.25) {
...
export function createIcosaSphere() {
  ...
  const faces = indices.map((tri, id) => triangleFace(id, tri.map((i) => raw[i])));
...
function rectangleFace(id, width, height, name) {
  return {
    ...
    v: [0, 1, 0],
    n: [0, 0, 1],
    verts2: [[-width / 2, -height / 2], [width / 2, -height / 2], [width / 2, height / 2], [-width / 2, height / 2]],
    edges: [{}, {}, {}, {}],
  };
}

function squareVerts(size) {
  return [[-size / 2, -size / 2], [size / 2, -size / 2], [size / 2, size / 2], [-size / 2, size / 2]];
}
...
function triangleFace(id, verts3) {
  ...
  return { id, name: `facet ${id}`, C, u, v, n, verts3: [a, b, c], verts2: [to2(a), to2(b), to2(c)], edges: [{}, {}, {}] };
}
...
function sampleField(x, y, width, height, faceId) {
  const u = x / width + 0.5;
  const v = y / height + 0.5;
  const hue = (u * 0.78 + v * 0.22 + 0.06 * Math.sin(u * Math.PI * 8)) % 1;
  const light = 0.48 + 0.2 * Math.sin((u + v) * Math.PI * 4);
  return sampleFromHue(hue, light, `${faceId}:${Math.floor(u * 12)}:${Math.floor(v * 8)}`);
}
